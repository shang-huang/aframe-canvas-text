...
AFRAME.registerComponent("canvas-text", {
  ...
  draw() {
    ...
    let _font = `${_data.size * _dpi}px ${_fontType}`;
    // support unicode character & line breaks 
    let _sentences = unicodeSentnece(_data.value).replace(/\\n/g, "\n").split("\n");
    ...
    _el.appendChild(_tempTextAreaEl);
    let _width = _tempTextAreaEl.offsetWidth;
    let _height = _tempTextAreaEl.offsetHeight;
    let _lineHeights = [];
    _tempTextAreaEl.childNodes.forEach(child => (child.nodeName.toLowerCase() === "span") && _lineHeights.push(child.offsetHeight));
    _el.removeChild(_tempTextAreaEl);
    // set canvas size
    ...
    _context.fillStyle = _data.color;
    let _fillHeight = 0;
    ...
    _el.hasAttribute("geometry") && _el.removeAttribute("geometry");
    _el.setAttribute("geometry", { primitive: "plane", width: _width * _objRatio, height: _height * _objRatio });
    _el.hasAttribute("material") && _el.removeAttribute("material");
    _el.setAttribute("material", { src: `#${_canvas.id}`, shader: "flat", npot: true, transparent: true });
  },
...
function unicodeSentnece(value) {
  let _uni_sentences = value.split("\\u");
  let _uni_sentence = "";
  _uni_sentences.forEach((_sentence, index) => {
    if (index === 0) {
      _uni_sentence += _sentence;
    } else {
      let _unicodePlan1Plan2 = getUnicodeCharFromSentence(_sentence, 5);
      if (/[\u{10000}-\u{2ffff}]/u.test(_unicodePlan1Plan2)) {
        ...
        let _unicodePlan0 = getUnicodeCharFromSentence(_sentence, 4);
